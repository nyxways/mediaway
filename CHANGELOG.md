...
irreversible) before the `native-assets-linux`/`native-assets-macos` jobs both failed for real
reasons — no npm/NuGet/PyPI/CPack packages or GitHub Release were ever published for 0.1.7. This
version fixes both root causes; everything else in the "What's new since 0.1.6" section below is
unchanged content, now actually able to ship.
