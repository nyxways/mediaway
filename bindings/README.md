...
| `screen_record` | — | video encode | screen capture | ✅ **yes**, via the GPU device factory (`mediaway_gpu_device_create`, `mediaway-device` ADR-0007) — Screen still needs a live GPU device handle (`ID3D11Device*`, no CPU fallback), but a C/Node caller can now create one itself instead of only rejecting the `NONE` placeholder. C#/Python/C++ parity is open follow-up work |
...
| `pipeline/screen_record.*` | [`examples/pipeline/screen_record.rs`](../examples/pipeline/screen_record.rs) | screen + mic capture → encode → MP4 | ✅ C, Node.js / native (browser) · 🚧 gap demo (C#, Python, C++) |
...
| `device/capture_screen.*` | [`examples/device/capture_screen.rs`](../examples/device/capture_screen.rs) | screen capture only | ✅ C, Node.js / native (browser) · 🚧 gap demo (C#, Python, C++) |
