...
- [x] `src/types.rs` — `Rational`/`CodecKind` `#[repr(C)]` mirrors + conversions
      ...
      `mediaway-pipeline-ffi`'s independent copy)
- [x] `src/buffer.rs` — `borrow_slice`/`leak_boxed_slice`/`reclaim_boxed_slice`,
      now `pub` (crosses a crate boundary)
- [x] `mediaway-container-ffi`, `mediaway-pipeline-ffi`, `mediaway-device-ffi` migrated
...
- `PixelFormat` mirroring — not decided by ADR-0015; `mediaway-pipeline-ffi` and
