...
//! session implementing [`mediaway_decoder::VideoDecoder`].
//!
...
use crate::cpu_readback;
use crate::decoder_hevc::HevcSession;
use crate::dpb::{Dpb, DpbSlot};
use crate::h264_params::{H264Pps, H264Sps, derive_pic_order_cnt_msb};
use crate::h264_slice::{
    H264SliceHeader, H264SliceType, apply_ref_pic_list_modifications, default_ref_pic_list0,
};
use crate::hevc_params::{HevcPps, HevcSps};
use crate::session::{
...
use crate::session_command_h264::{RecordParamsH264, build_picture_info, record_and_submit_h264};
use crate::zero_copy;
...
impl VulkanVideoDecoder {
    ...
    pub fn open(config: &VideoDecoderConfig) -> Result<Self, DecodeError> {
        ...
        if is_hevc {
            ...
            crate::decoder_hevc::scan_parameter_sets(
                &config.extra_data,
                &mut pending_sps,
                &mut pending_pps,
            );
    ...
    fn decode_slice_h264(&mut self, nal: &NalUnit, raw_nal: &[u8]) -> Result<(), DecodeError> {
        ...
        let output = match self.output {
            ...
            // `VideoOutputPreference` is `#[non_exhaustive]` — a future
            // variant this crate does not yet implement is rejected loudly
            // rather than silently falling back to a different path.
...
impl VideoDecoder for VulkanVideoDecoder {
    ...
    fn push_packet(&mut self, packet: &Packet) -> Result<(), DecodeError> {
        ...
        if self.codec == CodecKind::Hevc {
            return crate::decoder_hevc::push_packet_hevc(self, packet);
        }
