...
///
/// Crate-internal (not `mediaway_decoder::DecodeError` — see `session.rs`'s
...
impl Dpb {
    ...
    /// slot, per `mediaway_decoder::VideoDecoder`'s documented handle
    /// lifetime).
    ///
    /// # Errors
    /// Returns [`DpbError::InvalidSlotIndex`] if `index` is out of range.
    ...
    /// [`crate::h264_slice::default_ref_pic_list0`]'s list-building, or both
