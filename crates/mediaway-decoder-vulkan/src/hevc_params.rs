...
/// into [`crate::session::VulkanDecodeError`].
...
impl HevcNalUnitType {
    ...
    /// [`crate::hevc_slice::HevcSliceSegmentHeader::parse`].
    #[must_use]
...
pub struct HevcPps {
    ...
    /// segment header (needed by [`crate::hevc_slice`] to stay bit-aligned).
    pub output_flag_present_flag: bool,
    /// `num_extra_slice_header_bits` — count of `slice_reserved_flag[i]` bits
    /// to skip in the slice segment header (needed by [`crate::hevc_slice`]).
