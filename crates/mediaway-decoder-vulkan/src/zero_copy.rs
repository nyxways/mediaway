...
//! [`mediaway_decoder::VideoOutputPreference::ZeroCopyGpu`].
//!
...
//! **must** call [`crate::dpb::Dpb::mark_outstanding`] on that slot index
//! before returning it to the caller, and
//! [`crate::dpb::Dpb::clear_outstanding`] once the caller has recycled the
//! frame (see `mediaway_decoder::VideoDecoder::poll_frame`'s documented
...
use crate::session::VulkanDecodeError;
use crate::session_command::SessionResources;
