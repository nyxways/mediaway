//! Opaque audio capture handle and its C ABI functions — Microphone only.
//!
//! Handle shape and panic-safety strategy: `adr/0001-capture-c-abi.md` §2, §8. Split
//! out of the former unified `audio.rs` (Microphone+Loopback+`ProcessLoopback`) —
//! `adr/0004-domain-feature-split.md`: Loopback/`ProcessLoopback` moved to
//! `desktop_audio.rs`, under the `desktop` Cargo feature (they capture what the
//! desktop is already rendering, not a real input device — same grouping as the Rust
//! facade split). Local platform dispatch (§1) goes straight to
//! `mediaway-device-windows-audio`'s `WindowsWasapiCapture::open_microphone`, **not**
//! through the `mediaway-device-windows` orchestrator crate — that crate unconditionally
//! depends on the Camera/Desktop backends too, which would defeat this feature's
//! isolation.
...
use crate::types::{MediawayAudioCaptureConfig, MediawayDeviceAudioFrame, MediawayRational};

/// Opaque audio capture handle (`mediaway_audio_capture_t*` in the C header).
///
...
/// `MuxerHandle`/`DemuxerHandle` (`adr/0001-capture-c-abi.md` §2).
///
...
/// Build a default-microphone capture config.
#[unsafe(no_mangle)]
pub const extern "C" fn mediaway_audio_capture_config_microphone(
    time_base: MediawayRational,
) -> MediawayAudioCaptureConfig {
    MediawayAudioCaptureConfig {
        device_index: 0,
        time_base,
        sample_format: crate::types::MediawaySampleFormat::F32,
    }
}

/// Open a microphone capture session for `config`.
///
/// Three outcomes: (1) `Ok` — builds the handle, writes it to `*out_capture`; (2) a
/// normal `Err` (e.g. an unsupported sample-format) — no handle exists, `*out_capture`
/// is set to `NULL`, the matching status is returned; (3) a caught panic — same
/// `NULL`/[`MediawayDeviceStatus::InternalPanic`] shape as (2).
///
/// # Safety
///
/// `config` must be a valid, readable [`MediawayAudioCaptureConfig`] pointer.
/// `out_capture` must be a valid, writable, non-null out-parameter.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_audio_capture_open(
    config: *const MediawayAudioCaptureConfig,
    out_capture: *mut *mut AudioCaptureHandle,
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        let rust_config = AudioCaptureConfig {
            select: Select::Default,
            time_base: config.time_base.into(),
            sample_format: config.sample_format.into(),
        };
        open_audio_capture(&rust_config)
    }));

    match result {
        Ok(Ok(capture)) => {
            let handle = Box::new(AudioCaptureHandle {
                poisoned: false,
                inner: capture,
            });
...
/// Local `#[cfg(windows)]`/`#[cfg(target_os = "linux")]` audio dispatch — mirrors
/// `camera.rs`/`desktop_video.rs`'s shape (`adr/0001-capture-c-abi.md` §1). Goes
/// directly to `mediaway-device-windows-audio`, not the `mediaway-device-windows`
/// orchestrator (`adr/0004-domain-feature-split.md`).
...
/// # Safety
///
/// `capture` must be a live pointer returned by [`mediaway_audio_capture_open`].
/// `out_sample_rate`/`out_channels` must be valid, writable, non-null out-parameters.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_audio_capture_format(
    capture: *const AudioCaptureHandle,
    ...
    match result {
        ...
        // for an audio capture handle, so these are never actually `None` here.
...
/// released with [`mediaway_audio_frame_free`].
///
/// # Safety
///
/// `capture` must be a live pointer returned by [`mediaway_audio_capture_open`].
/// `out_frame` must be a valid, writable pointer to a [`MediawayDeviceAudioFrame`].
/// `out_has_frame` must be a valid, writable `bool` pointer.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_audio_capture_poll_frame(
    capture: *mut AudioCaptureHandle,
    out_frame: *mut MediawayDeviceAudioFrame,
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        Ok(Some(MediawayDeviceAudioFrame {
            pts: frame.pts,
...
/// Close an audio capture session, freeing its handle.
///
...
/// [`mediaway_audio_capture_open`] and not already passed to this function.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_audio_capture_close(
    capture: *mut AudioCaptureHandle,
) -> MediawayDeviceStatus {
    if capture.is_null() {
...
/// Free a frame returned by [`mediaway_audio_capture_poll_frame`].
///
...
/// # Safety
///
/// `frame` must be null or a valid, writable pointer to a [`MediawayDeviceAudioFrame`]
/// whose `data`/`data_len` were produced by [`mediaway_audio_capture_poll_frame`] and
/// not already freed.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_audio_frame_free(frame: *mut MediawayDeviceAudioFrame) {
    if frame.is_null() {
        return;
    }
    ...
    // `mediaway_audio_capture_poll_frame` (function contract).
