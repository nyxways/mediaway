...
/// Config for [`crate::camera::mediaway_camera_capture_open`] — plain value struct, no
...
/// Output of [`crate::camera::mediaway_camera_capture_poll_frame`] — owned; release
/// with [`crate::camera::mediaway_camera_frame_free`].
///
...
pub struct MediawayCameraFrame {
    ...
    /// Owned plane bytes. `NULL` after [`crate::camera::mediaway_camera_frame_free`].
    pub data: *mut u8,
...
///
/// Only `Screen` is reachable via [`crate::desktop_video::mediaway_desktop_capture_open`]
/// in this pass; `Window` deterministically returns
/// [`crate::MediawayDeviceStatus::Unsupported`] (`adr/0001-capture-c-abi.md` § Finding 2,
...
/// Config for [`crate::desktop_video::mediaway_desktop_capture_open`] — plain value
/// struct, no handle, no heap allocation, no free function.
///
/// `gpu_device` is mandatory for `Screen` (`adr/0003-gpu-handle-c-abi.md` §4) —
/// [`crate::desktop_video::mediaway_desktop_capture_open`] rejects a
...
/// Output of [`crate::desktop_video::mediaway_desktop_capture_poll_frame`] — release
/// with [`crate::desktop_video::mediaway_desktop_frame_free`].
///
...
pub struct MediawayDesktopFrame {
    ...
    /// [`crate::desktop_video::mediaway_desktop_frame_free`], and whenever
    /// `storage_kind == Gpu`.
    pub data: *mut u8,
...
/// Config for [`crate::audio::mediaway_audio_capture_open`] — plain value struct, no
/// handle, no heap allocation, no free function.
///
...
/// Output of [`crate::audio::mediaway_audio_capture_poll_frame`] — owned; release with
/// [`crate::audio::mediaway_audio_frame_free`].
///
...
/// Config for [`crate::desktop_audio::mediaway_desktop_audio_capture_open`] — plain
...
/// Output of [`crate::desktop_audio::mediaway_desktop_audio_capture_poll_frame`] —
/// owned; release with [`crate::desktop_audio::mediaway_desktop_audio_frame_free`].
///
/// Does not derive `Copy`/`Clone` for the same reason as [`MediawayCameraFrame`].
#[cfg(feature = "desktop")]
#[repr(C)]
...
pub struct MediawayDesktopAudioFrame {
    ...
    pub sample_format: MediawaySampleFormat,
    /// Owned interleaved sample bytes. `NULL` after
    /// [`crate::desktop_audio::mediaway_desktop_audio_frame_free`].
    pub data: *mut u8,
    /// Length of `data` in bytes.
    pub data_len: usize,
}
...
/// [`crate::hotplug::mediaway_device_hotplug_open`]'s `kinds` parameter and
...
pub enum MediawayDeviceKind {
    ...
    /// reasoning [`crate::MediawayDeviceStatus::UnknownError`] already applies to an
...
/// [`crate::hotplug::mediaway_device_hotplug_poll_event`] (release with
/// [`crate::hotplug::mediaway_device_hotplug_event_free`]); **borrowed**, valid only
