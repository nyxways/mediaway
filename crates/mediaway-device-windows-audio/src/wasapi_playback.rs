...
use mediaway_device_audio::{AudioPlayback, AudioPlaybackConfig, PlaybackError};
...
use crate::wasapi::{ComGuard, closed_audio_info, read_float_mix, resolve_endpoint};
...
///
/// Not CPU Zero-Copy, mirroring [`crate::wasapi::WindowsWasapiCapture`]'s
...
fn open_wasapi_render_client(
    ...
    let device = resolve_endpoint(&enumerator, eRender, select).map_err(|e| {
        notify_err(
            tx_info,
            match e {
                mediaway_device::CaptureError::AccessDenied => PlaybackError::AccessDenied,
                mediaway_device::CaptureError::Unsupported
                | mediaway_device::CaptureError::InvalidInput => PlaybackError::Unsupported,
