...
//!
//! [`enumerate_cameras`] (used by `mediaway_device_windows::enumerate`) builds each
//! [`mediaway_device::DeviceInfo::id`] from `MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK`
...
//! `mediaway_device_windows_audio::WindowsWasapiCapture` documents for WASAPI:
...
//! `mediaway_device_windows_desktop::WindowsScreenCapture`'s
...
use mediaway_device::{CaptureError, DeviceId, DeviceInfo, DeviceKind, Select};
...
fn resolve_camera_index(select: &Select) -> Result<usize, CaptureError> {
    match select {
        ...
        // `Select` is `#[non_exhaustive]` — any future variant is rejected
...
/// Live camera enumeration for `mediaway_device_windows::enumerate` (`DeviceKind::Camera`).
///
