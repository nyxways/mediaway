...
use crate::session::{EncodeDevice, SessionResources, VulkanEncodeSessionError};
use crate::session_command::{record_upload_and_barriers, submit_and_readback};
...
/// for one HEVC IDR picture — mirrors
...
fn record_video_coding_hevc(
    ...
    // (`src_resource`/`setup_slot`/`dpb_resource`/the HEVC picture-info chain
