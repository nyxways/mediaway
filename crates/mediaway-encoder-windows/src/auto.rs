...
//!
//! Types: [`mediaway_encoder::auto`]. This module owns the session so the facade
...
use mediaway_encoder::auto::{AutoVideoEncodeConfig, Backend, BackendSelection, EncodePathClass};
use mediaway_encoder::{EncodeError, VideoEncoder, VideoInputPreference};
use std::fmt;

use crate::WindowsVideoEncoder;
...
pub struct AutoVideoEncoder {
    ...
    /// bridged `ID3D11Device`/`ID3D11Texture2D` (see [`crate::D3d12SharedEncodeBridge`]),
    ...
    #[cfg(windows)]
    bridge: Option<crate::D3d12SharedEncodeBridge>,
}
...
impl AutoVideoEncoder {
    ...
    ///   [`crate::D3d12SharedEncodeBridge`] — one GPU→GPU copy per frame, see
    ...
    fn try_nvenc(config: &AutoVideoEncodeConfig) -> Result<Self, EncodeError> {
        let low = config.to_low_level(VideoInputPreference::CpuUploadOk, None);
        let inner = mediaway_encoder_nvenc::NvencVideoEncoder::open(&low)?;
        Ok(Self::with_path(
            EncodePathClass::CpuUpload,
            Backend::Nvenc,
    ...
    fn try_quicksync(config: &AutoVideoEncodeConfig) -> Result<Self, EncodeError> {
        let low = config.to_low_level(VideoInputPreference::CpuUploadOk, None);
        let inner = mediaway_encoder_quicksync::QuickSyncVideoEncoder::open(&low)?;
        Ok(Self::with_path(
            EncodePathClass::CpuUpload,
            Backend::QuickSync,
    ...
    /// [`crate::D3d12SharedEncodeBridge`] for the per-frame `CopyResource` contract.
    #[cfg(windows)]
    fn try_gpu_copy(
        ...
        let bridge =
            crate::D3d12SharedEncodeBridge::open(d3d12_device, config.width, config.height)?;
    ...
    /// [`crate::D3d12SharedEncodeBridge::as_dx11_handle`]) can encode it. See
    ...
    /// shared texture — see [`crate::D3d12SharedEncodeBridge`].
    #[cfg(windows)]
    #[must_use]
...
/// [`mediaway_encoder::capability`](../../mediaway-encoder/src/capability.rs).
///
...
#[must_use]
pub fn support(codec: CodecKind) -> Vec<mediaway_encoder::capability::EncoderCapability> {
    use mediaway_encoder::capability::{EncodeSupport, EncodeUnavailable, EncoderCapability};
