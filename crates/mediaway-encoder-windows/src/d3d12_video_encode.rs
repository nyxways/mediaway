...
//! key frame).
//!
//! This is a **different** path than [`crate::D3d12SharedEncodeBridge`]: that bridge feeds
...
//!
//! **Not wired into [`crate::WindowsVideoEncoder`] / `auto` yet** — this module is
...
#![allow(
    dead_code,
    reason = "every open()/push_frame() call path here is only reachable from this module's own #[cfg(test)] tests today (not wired into crate::WindowsVideoEncoder yet, see module doc) — rustc's dead_code pass sees a live call graph under cfg(test) but flags the same items in the plain (non-test) lib build; same root cause as the unreachable_pub allow above, resolved together once a later pass wires this backend into the public API"
)]
