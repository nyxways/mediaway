# ADR-0006: VA-API DMA-BUF Zero-Copy encode input
...
`mediaway-decoder`'s `adr/linux/0006-vaapi-dmabuf-zero-copy-output.md` (read first; this ADR
...
hence ADR-0006 (decoder)'s DPB `outstanding` re-port. Encode's imported input surface has a
...
the `Picture<S, T>` typestate chain) is hard-typed to `Surface<()>` in the pre-ADR-0006 code.
...
- Companion: `mediaway-decoder` `adr/linux/0006-vaapi-dmabuf-zero-copy-output.md` (defines
