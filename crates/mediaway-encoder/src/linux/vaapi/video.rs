...
//! [ADR-0003](../../adr/linux/0003-vaapi-dmabuf-zero-copy-input.md) adds
//! [`VideoInputPreference::ZeroCopyGpu`] DMA-BUF import (`super::dmabuf`) as a second input
//! source alongside CPU upload — see [`VaapiVideoEncoder::push_frame`].
...
pub(crate) struct VaapiVideoEncoder {
    ...
    /// ADR-0003). `push_frame` rejects a frame whose `VideoFrameStorage` does not match this.
...
impl VaapiVideoEncoder {
    ...
    pub(crate) fn open(config: &VideoEncoderConfig) -> Result<Self, EncodeError> {
        ...
        match config.input {
            ...
            // `push_frame` call differs (ADR-0003 § Decision).
    ...
    fn open_cpu(config: &VideoEncoderConfig) -> Result<Self, EncodeError> {
        ...
        // is single-use and dropped after the call that encodes it (ADR-0003 § "Why no
    ...
    /// `push_frame_dmabuf` (ADR-0003) — `ZeroCopyGpu` forces `effective_gop_size <= 1`
    ...
    /// Zero-Copy DMA-BUF import path (`VideoInputPreference::ZeroCopyGpu`, ADR-0003) — imports
    ...
    /// surface is never written back into `self.surfaces` (it is not pooled — see ADR-0003 §
...
impl VideoEncoder for VaapiVideoEncoder {
    ...
    fn push_frame(&mut self, frame: &VideoFrame) -> Result<(), EncodeError> {
        ...
        // ADR-0003 § Decision: the source surface's provenance must match `open()`'s declared
