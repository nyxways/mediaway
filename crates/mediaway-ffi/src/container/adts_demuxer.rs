//! Opaque ADTS demuxer handle and its C ABI functions.
//!
//! Dedicated shape, mirroring `mediaway_ogg_demuxer_t` — see
//! `crates/mediaway-ffi/src/container/ogg_demuxer.rs` module docs for why this stays a
//! dedicated handle instead of a `DemuxerState` variant, and `adr/0004-ogg-adts-c-abi.md`.
...
/// Opaque ADTS demuxer handle (`mediaway_adts_demuxer_t*` in the C header).
///
...
pub struct AdtsDemuxerHandle {
    poisoned: bool,
    inner: adts::Demuxer,
}

/// Create a new, empty ADTS demuxer.
///
/// Returns null only if a panic was caught during construction (defensive;
/// `adts::Demuxer::new()` is simple enough that this should not trigger in practice).
#[unsafe(no_mangle)]
pub extern "C" fn mediaway_adts_demuxer_create() -> *mut AdtsDemuxerHandle {
    let built = catch_unwind(AssertUnwindSafe(|| AdtsDemuxerHandle {
        poisoned: false,
        inner: adts::Demuxer::new(),
    }));
...
/// Feed ADTS elementary-stream bytes into the demuxer.
///
/// # Safety
///
/// `demuxer` must be a live pointer returned by [`mediaway_adts_demuxer_create`]. `data`
/// must be valid for reads of `len` bytes for the duration of this call (or null with
/// `len == 0`).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_adts_demuxer_push_bytes(
    demuxer: *mut AdtsDemuxerHandle,
    data: *const u8,
    len: usize,
) -> MediawayStatus {
    if demuxer.is_null() {
...
/// Number of streams discovered so far — `0` or `1` (ADTS carries a single implicit
/// stream, recognized once the first frame's header has been parsed).
///
...
/// # Safety
///
/// `demuxer` must be a live pointer returned by [`mediaway_adts_demuxer_create`].
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_adts_demuxer_stream_count(
    demuxer: *const AdtsDemuxerHandle,
) -> usize {
    if demuxer.is_null() {
        return 0;
    }
...
/// Get stream info by index (always index `0` once the first frame has been parsed).
///
/// # Safety
///
/// `demuxer` must be a live pointer returned by [`mediaway_adts_demuxer_create`].
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_adts_demuxer_stream_at(
    demuxer: *mut AdtsDemuxerHandle,
    index: usize,
...
/// Pop the next demuxed packet (one AAC frame), if any is ready.
///
/// `pts`/`duration` are synthesized from a running 1024-samples-per-frame count — ADTS
/// carries no per-frame timing of its own (see `mediaway-container::adts` module docs).
///
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_adts_demuxer_poll_packet(
    demuxer: *mut AdtsDemuxerHandle,
...
/// Close and free an ADTS demuxer handle.
///
...
/// [`mediaway_adts_demuxer_create`] and not already passed to this function.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_adts_demuxer_close(demuxer: *mut AdtsDemuxerHandle) {
    if demuxer.is_null() {
        return;
    }
