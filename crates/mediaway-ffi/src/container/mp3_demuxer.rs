//! Opaque MP3 (MPEG Layer III) demuxer handle and its C ABI functions.
//!
//! Dedicated shape, mirroring `mediaway_adts_demuxer_t` — see that module's docs for why
//! this stays a dedicated handle instead of a `DemuxerState` variant, and
//! `adr/0007-mp3-c-abi.md`.
...
/// Opaque MP3 demuxer handle (`mediaway_mp3_demuxer_t*` in the C header).
///
...
pub struct Mp3DemuxerHandle {
    poisoned: bool,
    inner: mp3::Demuxer,
}

/// Create a new, empty MP3 demuxer.
///
/// Returns null only if a panic was caught during construction (defensive;
/// `mp3::Demuxer::new()` is simple enough that this should not trigger in practice).
#[unsafe(no_mangle)]
pub extern "C" fn mediaway_mp3_demuxer_create() -> *mut Mp3DemuxerHandle {
    let built = catch_unwind(AssertUnwindSafe(|| Mp3DemuxerHandle {
        poisoned: false,
        inner: mp3::Demuxer::new(),
    }));
...
/// Feed MPEG audio elementary-stream bytes into the demuxer.
///
/// # Safety
///
/// `demuxer` must be a live pointer returned by [`mediaway_mp3_demuxer_create`]. `data`
/// must be valid for reads of `len` bytes for the duration of this call (or null with
/// `len == 0`).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_mp3_demuxer_push_bytes(
    demuxer: *mut Mp3DemuxerHandle,
    data: *const u8,
    len: usize,
) -> MediawayStatus {
    if demuxer.is_null() {
...
/// recognized once the first frame's header has been parsed).
///
...
/// # Safety
///
/// `demuxer` must be a live pointer returned by [`mediaway_mp3_demuxer_create`].
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_mp3_demuxer_stream_count(
    demuxer: *const Mp3DemuxerHandle,
) -> usize {
    if demuxer.is_null() {
        return 0;
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_mp3_demuxer_stream_at(
    demuxer: *mut Mp3DemuxerHandle,
    index: usize,
...
/// Pop the next demuxed packet (one Layer III frame), if any is ready.
///
/// `pts`/`duration` are synthesized from a running samples-per-frame count — MPEG audio
/// carries no per-frame timing of its own (see `mediaway-container::mp3` module docs).
///
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_mp3_demuxer_poll_packet(
    demuxer: *mut Mp3DemuxerHandle,
...
/// Close and free an MP3 demuxer handle.
///
...
/// [`mediaway_mp3_demuxer_create`] and not already passed to this function.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mediaway_mp3_demuxer_close(demuxer: *mut Mp3DemuxerHandle) {
    if demuxer.is_null() {
        return;
    }
