...
fn take_extra_data(info: &mut mediaway_ffi::pipeline::MediawayAudioStreamInfo) -> Bytes {
...
fn audio_encode_roundtrips_to_fmp4() {
    ...
    let config = mediaway_ffi::pipeline::mediaway_audio_encode_config_aac(
        SAMPLE_RATE,
        MediawayRational {
            num: 1,
            den: SAMPLE_RATE,
    ...
    let mut session: *mut mediaway_ffi::pipeline::AudioEncodeSessionHandle = std::ptr::null_mut();
    let status = unsafe {
        mediaway_ffi::pipeline::mediaway_audio_encoder_open(&raw const config, &raw mut session)
    };
    assert_eq!(
        status,
    ...
    for (i, pcm) in sine_frames().into_iter().enumerate() {
        ...
        let status = unsafe {
            mediaway_ffi::pipeline::mediaway_audio_encode_session_push_pcm(
                session,
                &raw const frame,
            )
        };
    ...
    let status = unsafe { mediaway_ffi::pipeline::mediaway_audio_encode_session_flush(session) };
    assert_eq!(status, MediawayPipelineStatus::Ok);

    // ── stream info: codec config a muxer needs ────────────────────────────
    let mut info = mediaway_ffi::pipeline::MediawayAudioStreamInfo::default();
    let status = unsafe {
        mediaway_ffi::pipeline::mediaway_audio_encode_session_stream_info(session, &raw mut info)
    };
    assert_eq!(status, MediawayPipelineStatus::Ok);
    ...
    loop {
        ...
        let mut has = false;
        let status = unsafe {
            mediaway_ffi::pipeline::mediaway_audio_encode_session_poll_packet(
                session,
                &raw mut packet,
                &raw mut has,
            )
        ...
        unsafe { mediaway_ffi::pipeline::mediaway_pipeline_ffi_packet_free(&raw mut packet) };
    }
    ...
    unsafe { mediaway_ffi::pipeline::mediaway_pipeline_ffi_stream_info_free(&raw mut info) };
