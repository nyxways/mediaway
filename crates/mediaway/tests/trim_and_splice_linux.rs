//! Real decode → trim → splice → re-encode round trip (Linux, H.264 VA-API CPU path).
//!
//! Mirrors `tests/trim_and_splice_windows.rs`: encodes two synthetic clips (distinct
//! per-frame luma so segment identity/order survives compression), muxes/demuxes/
//! decodes each through `mediaway-container`, drops the first/last frame of each
//! (**trim**), concatenates the remaining frames with renumbered contiguous
//! timestamps (**splice**), re-encodes the result, and decodes the output to verify
//! frame count, order, and content.
//!
//! `mediaway-encoder-linux`/`mediaway-decoder-linux` are VA-API (`cros-libva`)
//! backends with **zero real-hardware verification** (see each crate's ADR-0001):
//! opening a VA-API display fails in any environment without a real
//! `/dev/dri/renderD*` device (WSL2, most CI, this repo's dev box included), so this
//! test is expected to always skip via the honest `eprintln!("skip: ...")` path below
//! rather than exercise real VA-API encode/decode.
...
/// Encode `frames` (H.264 VA-API CPU-upload path) and mux into a single-track fMP4.
/// `None` on `LinuxVideoEncoder::open` failure (no VA-API display/driver available in
/// this environment — expected on WSL2 and most CI/dev boxes).
fn encode_frames(frames: &[VideoFrame]) -> Option<Vec<u8>> {
    ...
    let mut enc = match LinuxVideoEncoder::open(&cfg) {
        Ok(e) => e,
        Err(e) => {
            eprintln!("skip: LinuxVideoEncoder::open (VA-API) failed ({e:?})");
            return None;
        }
    };
    for f in frames {
    ...
    // instead of a real AVCDecoderConfigurationRecord. Same convention as the
    // Windows test — see `docs/ai/wiki/pipeline/trim-and-splice.md`.
...
/// Demux `fmp4_bytes`, open the H.264 VA-API CPU decode path against the demuxed
/// track's extradata, and decode every packet. `None` on `LinuxVideoDecoder::open`
/// failure (no VA-API display/driver available in this environment).
fn decode_fmp4(fmp4_bytes: &[u8]) -> Option<Vec<VideoFrame>> {
    ...
    let mut dec = match LinuxVideoDecoder::open(&cfg) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("skip: LinuxVideoDecoder::open (VA-API) failed ({e:?})");
            return None;
        }
    };
