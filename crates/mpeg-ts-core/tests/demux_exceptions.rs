...
//! stream count and demux access-unit count (`nb_read_packets`, else `nb_frames`).
...
/// `(streams, demux_access_units)` from ffprobe.
///
/// Prefers `nb_read_packets` (edit-list–expanded demux count) when present;
/// falls back to `nb_frames`. `ffprobe` prints every MPEG-TS stream twice in
/// CSV mode (once from the program-grouped view, once from the flat stream
/// list — confirmed via `-of json`, which nests the same stream under both
/// `programs[].streams` and top-level `streams`); collapse consecutive
/// duplicate lines rather than double-counting.
fn ffprobe_counts(path: &Path) -> Option<(usize, usize)> {
    ...
    for line in text.lines() {
        ...
        if last_line == Some(line) {
            continue; // MPEG-TS program/stream CSV duplicate — see doc comment
        }
        last_line = Some(line);
...
fn demux_chunked(bytes: &[u8]) -> (usize, usize) {
    let mut d = Demuxer::new();
    // Push all bytes first to ensure PAT/PMT are available
    for chunk in bytes.chunks(17) {
        d.push_bytes(chunk);
    }
    // Poll access units to trigger PAT/PMT parsing and get stream count
    let mut access_units = 0usize;
    while d.poll_access_unit().is_ok_and(|au| au.is_some()) {
        access_units += 1;
    }
    // Flush any remaining access unit that was still accumulating
    let final_units = d.finish();
    access_units += final_units.len();
    let streams = d.streams().len();
    (streams, access_units)
}
...
fn demux_empty_input_yields_nothing() {
    let (streams, access_units) = demux_chunked(&[]);
    assert_eq!(streams, 0);
    assert_eq!(access_units, 0);
}

#[test]
fn demux_truncated_buffer_does_not_panic() {
    demux_chunked_no_panic(&[0x47, 0x00, 0x00, 0x00, 0x00]);
}
...
fn demux_fate_manifest_samples() {
    ...
    for ent in &entries {
        ...
        if ent.mode == FateMode::OracleCompare && probe {
            ...
            let (mw_streams, mw_access_units) = demux_chunked(&bytes);
            assert_eq!(
            ...
            assert_eq!(
                mw_access_units, ff_count,
                "access-unit count mismatch on {}: mediaway={mw_access_units} ffprobe={ff_count} (nb_read_packets preferred)",
                ent.rel
            );
        }
        seen += 1;
