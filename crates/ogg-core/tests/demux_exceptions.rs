...
fn demux_chunked(bytes: &[u8]) -> (usize, usize) {
    ...
    let mut packets = 0usize;
    while let Ok(Some(pkt)) = d.poll_packet() {
        serials.insert(pkt.serial);
        packets += 1;
    }
    (serials.len(), packets)
}
...
fn demux_empty_input_yields_nothing() {
    let (streams, packets) = demux_chunked(&[]);
    assert_eq!(streams, 0);
    assert_eq!(packets, 0);
}
...
fn demux_truncated_header_does_not_panic() {
    demux_chunked_no_panic(&[0x4f, 0x67, 0x67, 0x53, 0x00, 0x00]);
}
...
fn demux_fate_manifest_samples() {
    ...
    for ent in &entries {
        ...
        if ent.mode == FateMode::OracleCompare && probe {
            ...
            let (mw_streams, mw_packets) = demux_chunked(&bytes);
            assert_eq!(
