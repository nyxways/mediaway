...
| `mediaway-container` | `crates/mediaway-container` | Traits + Mediaway `mp4`/`webm`/`adts-core`/`wav`/`mp3`/`ogg`/`flv-core`/`ts` |

Product callers: `mediaway_container::{mp4, webm, adts-core, wav, mp3, ogg, flv-core, ts}`.
Freestanding / no Mediaway types: `iso_bmff`, `ebml_webm`, `riff_wave`,
`adts-core`, `mpeg_audio`, `ogg`, `flv-core`, `mpeg_ts`.
