...
| Demux exceptions | Synthetic always · FATE via `MEDIAWAY_FATE_SAMPLES` + `fate_manifest.txt` (`oracle_compare` / `must_not_panic`) — every container crate (`iso-bmff`, `ebml-webm`, `riff-wave-core`, `adts-core`, `mpeg-audio`, `ogg`, `flv`, `mpeg-ts-core`) |
