...
box. `mediaway-encoder-windows/tests/av_fmp4_smoke.rs` does this "wrong" thing too but
never decodes its own output, so it never caught it.

`mediaway_pipeline::platform::AutoDecoder::open`/`AutoEncoder::open` now also dispatch
