...
| [linux-encode](linux-encode.md) | VA-API H.264 CPU-upload encode backend; real P-frame GOP implemented (ADR-0002); AV1 `KEY_FRAME`+GOP design-only (ADR-0003, blocked on a `cros-libva` gap); zero HW verification caveat |
...
| [apple-audio](apple-audio.md) | `AudioToolbox` `AudioConverter` AAC-LC encode + decode — implemented, zero compile verification; first AAC decoder in this workspace at all |
