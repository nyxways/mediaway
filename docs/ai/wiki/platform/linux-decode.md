...
- Codec: H.264 baseline/main (`VAProfileH264ConstrainedBaseline`/`VAProfileH264Main`), HEVC Main
  (`VAProfileHEVCMain`, ADR-0003), AV1 Main `KEY_FRAME`-only (`VAProfileAV1Profile0`, ADR-0003),
  VP9 Profile 0 `KEY_FRAME` + general `INTER_FRAME` (`VAProfileVP9Profile0`, ADR-0004) — all
  `VAEntrypointVLD`. Dispatched via `vaapi::VaapiVideoDecoder`, an enum over the four per-codec
  decoders (no `Box<dyn VideoDecoder>`); `LinuxVideoDecoder::open` picks a variant from
  `config.codec`.
...
- ADR: [0003 (HEVC)](../../../../crates/mediaway-decoder/adr/linux/0003-vaapi-hevc-p-slice-dpb.md) —
  ...
  source existed** (Vulkan's own HEVC decode is IDR-only) — fresh single-slot `HevcDpb`
  (`vaapi/hevc_dpb.rs`, reuses `vaapi/dpb.rs::derive_pic_order_cnt_msb` directly — ITU-T H.265
  §8.3.1 shares H.264 §8.2.1.1's MSB/LSB formula) and a slice-header parser extended well past
  `vulkan::hevc_slice.rs`'s own stopping point (VA-API needs many explicit per-slice fields —
  SAO, temporal-MVP, merge-cand count, QP deltas — Vulkan's hardware parses from raw bytes
  directly). No `hevc_vps.rs` needed (`PictureParameterBufferHEVC` has no VPS-derived field).
- ADR: [0003 (AV1)](../../../../crates/mediaway-decoder/adr/linux/0003-vaapi-av1-key-frame-decode.md) —
  AV1 `KEY_FRAME`-only decode, single tile, Main profile, every optional coding tool
  (segmentation/film grain/CDEF/loop restoration/superres/warped motion) rejected if signaled. No
  AV1 decode existed anywhere in this workspace to port from — the bitstream parser is
  spec-derived, cross-checked against `mediaway-encoder::windows::d3d12_video_encode::
  bitstream_av1`'s OBU writer (inverse direction) and `mediaway_sw::h264::BitReader`. No
  `cros-libva` packed-header gap on the decode side (unlike its encoder sibling).
- ADR: [0004 (VP9)](../../../../crates/mediaway-decoder/adr/linux/0004-vaapi-vp9-key-frame-and-inter-decode.md) —
  ...
  `FFmpeg`'s `vaapi_vp9.c`). Broader scope than the AV1 sibling: VP9 entropy adaptation is
  driver-internal, reference model is a flat 8-slot array (2 fields/slot). Spec-derived
  `uncompressed_header()` parser copied verbatim from the real primary VP9 spec text
  (`pdftotext`-extracted). Confirmed VP9's `s(n)` (magnitude then separate sign bit) differs from
  AV1's `su(n)` (sign embedded in the same field).

All four codecs above: compile/clippy/test-verified on real WSL2 Linux only — **zero real
VA-API hardware verification**, same caveat for every ADR in this folder.
