...
Optional corpus from the **FFmpeg FATE suite** (third-party samples; not committed). Every container crate with a demuxer carries its own manifest + test — `iso-bmff` (MP4), `ebml-webm` (WebM/Matroska), `riff-wave-core` (WAV), `adts-core` (raw AAC), `mpeg-audio` (MP3/Layer III), `ogg` (Vorbis/Opus transport), `flv`, `mpeg-ts-core`:
